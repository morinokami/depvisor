...
- **Package-manager detection is pinned at preflight** (`core/pm.ts`, called once in `update-one.ts` preflight against the trusted base tree; also by `install-target.ts` and `dev/scan.ts`).
  ...
  - **Workspace monorepos (npm/pnpm `workspaces`) are supported**: collect populates `Candidate.locations` (repo-relative workspace paths; root = `""`) and `pm.updateInstruction(candidates)` builds workspace-scoped commands from it (`npm install … -w <ws>`, or a single `pnpm -r update …`), so an update only ever touches the workspace(s) that already declare a dependency, never the root. bun/yarn workspaces are out of scope (bun candidates are always `[""]`).
