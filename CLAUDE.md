...
- **Package-manager detection is pinned at preflight** (`core/pm.ts`, called once in `update-one.ts` preflight against the trusted base tree; also by `install-target.ts` and `dev/scan.ts`).
  ...
  - The detected `pm` drives collect, verify `run` prefixes, the agent's update-command instruction, and (via `pm.lockfiles`) the lockfile half of the two-commit split's bump commit — `git.ts`'s `manifestBumpPaths` pairs those lockfiles with every changed `package.json` (root or workspace), matched by basename.
