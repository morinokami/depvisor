...
| `open-pr-blocked`    | An expected human intervention stopped the push: someone pushed to the PR branch (depvisor refuses to force-push over their commits — including during conflict refresh; merge or close the PR to hand the branch back), or, in conflict-refresh-only mode, the target PR was merged/closed while the run was in flight, so there was nothing left to refresh and no new PR is opened. |
