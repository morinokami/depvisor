...
import type { UpdatePlan } from "./bump.ts";
import { pnpmUpdatePlan } from "./pm-pnpm.ts";
import type { Candidate } from "./types.ts";

/**
 * Package-manager detection and per-PM command table. The JS package semantics
 * are shared; only the concrete commands and lockfiles differ. The npm and bun
 * planners are inline; pnpm's — the one that needs workspace enumeration and
 * catalog classification — lives in pm-pnpm.ts.
 *
