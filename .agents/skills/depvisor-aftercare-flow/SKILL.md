...
8. **Hand-off** — tokenless `bundle-payload.ts` packages the repair commits into a git bundle next to the payload, and the analyze action uploads `pr-preview/` as a workflow artifact. Commits travel as data because the two jobs never share a runner: target scripts can taint the analyze runner's files (`$GITHUB_PATH`, `$GITHUB_ENV`, `BASH_ENV`), so nothing token-holding may execute there.

9. **Publish** (token-holding `publish.ts` → `github.ts:publishAftercare`, in its OWN job via `publish/action.yml` on a fresh runner): build a from-scratch repository off the trusted remote URL, verify the PR is still open on the trusted head ref, compare-and-swap — push the repair only while the remote tip equals `expectedHeadSha` (a moved head or closed PR is the green `publish-blocked`), re-verify the pushed range structurally (bundle tip = payloaded repair sha, descent, sentinel committers, `repairScopeViolations`-clean diff), then upsert the ONE marker-deduplicated report comment. PR identity and remote URL come from trusted env; the artifact must agree. A real error is `publish-failed` (red).

## The status vocabulary

- Analyze, green: `report-prepared`, `repair-prepared`, `not-an-update-pr`, `deferred`.
- Analyze, red (fails the job): `in-progress` (the crash marker the workflow writes up front — only a graceful finish upgrades it), `bad-base-ref`, `bad-head-ref`, `bad-pr-number`, `bad-language`, `not-a-repo-root`, `persisted-credentials`, `dirty-tree`, `unsupported-package-manager`, `ambiguous-package-manager`, `missing-base-ref`, `no-verify-scripts`, `baseline-red`, `reinstall-unavailable`, `reinstall-failed`, `verification-failed`, `scope-violation`, `unexpected-commits`, `no-structured-result`.
- Publish (its OWN action outputs, never in the status file): `published`, `publish-blocked` (green), `no-payload` (green), `publish-failed` (red).

`aftercare.ts` writes `pr-preview/status.json` incrementally under the red `in-progress` marker, so a mid-run crash fails the job instead of impersonating a green run; `report-status.ts` projects it into annotations, the step summary, and the analyze action's machine-shaped outputs. The publish job writes its own outputs (`status`/`pushed`/`comment_url`) from `publish.ts` directly — `repaired` (analyze) means prepared, `pushed` (publish) means landed.
