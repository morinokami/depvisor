...
| `bump-failed`                                                                                                                          | The deterministic dependency bump or its install failed for a group (e.g. an npm `ERESOLVE`, a failed pnpm catalog edit, or a hung install); the summary names the failing step and shows the output tail. The fixer cannot help (it may not touch manifests), so the group is skipped — usually a real dependency conflict to resolve. |
...
| `unexpected-commits` / `scope-violation`                                                                                               | The fixer stepped outside its box (ran git, or touched manifests/lockfiles/denied paths); nothing was trusted or committed. Re-run; a recurrence is worth an issue.                                                                                                                                                                     |
